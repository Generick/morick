...
function ajax_stepless_classification($arg = array())
{
	...
	(
		...
		'table' => '',
		'pid' => $CI->uri->segment(4),
		'id' => $CI->uri->segment(5),
		'display_type' => $CI->uri->segment(6)
	);
	...
	$datas = $CI->m_common->get_all($new_arg['table'], $where, $new_arg['id_name'] . ', ' . $new_arg['field_name']);
	$str = '';
	foreach($datas as $row)
	{
		$str .= "sel.options[sel.options.length] = new Option('{$row[$new_arg['field_name']]}', '{$row[$new_arg['id_name']]}');";
	}
	$pp_data = $CI->m_common->get_one($new_arg['table'], array($new_arg['id_name'] => $new_arg['pid']), $new_arg['pid_name']);
	$ppid = '';
	if($pp_data)
	{
...
/* Location: ./application/helpers/my_stepless_classification_helper.php */